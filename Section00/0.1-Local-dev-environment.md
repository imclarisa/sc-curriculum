...
We will show you how to setup all of these tools.
...
### **Everyone Do The Following System Checks:-**
...
And, in your Git client, use the SSH based GitHub URL to clone your repos.
...
> **For fully detailed steps and instructions on setting up an SSH key for GitHub see [Section00\0.1.1-GitHub_SSH_Key.md](0.1.1-GitHub_SSH_Key.md)**
...
When signing up for these new services, most of them will send email messages to confirm your new account. Some might require additional verification actions.
...
- Stephanie G. will fill you in on all the detail, and send you an invite to join the correct team project etc.
...
### **`Install Insomnia`** -- [Insomnia.rest](https://insomnia.rest/)

Open the Insomnia website, and click on the button that says 'Get Started for Free'
...
- The The `operating system` (OS) handles ...
...
- In a `CLI` \(A.K.A. "Console", "Command Line", "Terminal", or "Shell"\), the user types commands using the keyboard to tell the computer to take an action. The computer will often display the results of the operation to the `console` or screen.
