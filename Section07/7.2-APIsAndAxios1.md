...
router.hooks({
  before: (done, params) => {
    const view = params && params.data.hasOwnProperty("view") ? capitalize(params.data.view) : "Home";
...
Weather in ${store.weather.city} ${kelvinToFahrenheit(store.weather.temp)}F, feels
    like ${kelvinToFahrenheit(store.weather.feelsLike)}F
```
