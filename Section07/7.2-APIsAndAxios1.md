...
So far, we haven't had to consider security as we haven't had to use API keys/tokens.  You NEVER want to hard code our API keys/tokens into our JS, which is then pushed to GitHub for all the world to see forever, even if you remove it from the code it will still be available in Git history.

Luckily, we have a third-party module that can create hidden **environment variables**: [`dotenv`](https://www.npmjs.com/package/dotenv).  Even more fortunate it is included when we use Parcel, so there is no need to install it but just in case, here is the command necessary.
```bash
npm install dotenv
```
If we are not using parcel then we would need to import and run the `config()` method as follows, which would give you access to the `process.env` global variables as later described.
```javascript
import dotenv from 'dotenv';
dotenv.config();
```

1. Create a `.env` file in the root directory and add our OpenWeatherMap API key and the Pizza place API key to a line:
...
2. We never want to give GitHub access to our `.env` file, so we need to add `.env` to our `.gitignore`.

3. Then we can access our environment variables from the `process.env` Object.
...
6. That's more like it! How can we store this in state? We first use `axios` to get the data from our API endpoint, then we can use that data to populate a new piece of state. In our example, we are creating a new key (`weather`) in `store.Home` that holds some weather data as an Object.
    ...
    if (view === "Home") {
      axios
        ...
        .then(response => {
          ...
          state.Home.weather.temp = kelvinToFahrenheit(response.data.main.temp);
          state.Home.weather.feelsLike = kelvinToFahrenheit(
...
7. Go to 'views/Home.js' and import this information so it is displayed on your HomePage. Remember to call state in your default export function.

```javascript
  <h3>
    Temperature in ${st.weather.city} is ${st.weather.temp}F. It feels like
