...
2. At the top of your root index.js file, add the following import statement: `import axios from "axios"`
    - At this point, this is what the top of your root index.js should look like:
...
3. After your `afterRender()` function but before your `router.on` function, let's create a new function with `router.hooks` and add the axios call inside of it as shown below.
      ...
      // Add a switch case statement to handle multiple routes
        switch (view) {
          case "Home":
            axios
              .get(
                // Replace {your api key} with your own key from openweathermap
                `https://api.openweathermap.org/data/2.5/weather?q=st%20louis&appid={your api key}`
              )
              .then(response => {
    ...
    > **NOTE This is only a brief explanation of `router.hooks`, you can find more information here: [navigo - Hooks documentation](https://github.com/krasimir/navigo/blob/e18744d8f79ec6fb654d426831a519cfc79aba44/DOCUMENTATION.md#hooks). We will also go more in depth about what the `switch case` is doing later on, but for now just understand that this is what allows us to handle multiple API calls for multiple routes.

4. Go to your console and run `npm run serve`. Open your dev tools in the browser after navigating to `localhost:1234` and click on the console tab. What is now printed out?
...
### **Adding Axios to the SPA and Storing the Data in State**

Before we cover storing the data in state, let's follow these steps to setup axios in our Class SPA:

- If you have not already, run this command in your terminal: `npm install --save axios`
- At the top of your root index.js file, add the following import statement: `import axios from "axios"`

>**HINT**: You will need to do these steps in your Capstone for the Week 7 Homework!
...
Now that that the information is collected from the API, we can now format it! This doesn't just mean saving it to state, we can manipulate it as well!
...
Now that our API Key is secure, we can learn about storing the data into state.
