...
2. Let's begin to construct our state by creating a directory named `state` in the root level of our repo.

3. Inside of `state`, create the following JS files:
...
6. Inside `state/index.js`, let's aggregate each of our modules with one-line import/export statements.
...
9. Our render function is accepting a piece of state and calling our functional components with pieces of state, however, we need to change our functional components to know what to do with that information.
   ...
   - Let's transform the way our NAV works. We'll "refactor our code" (rewrite it in a better format), make it easier to maintain, and make if more 'functional'.
     In our store let's create a `state/Links.js` file, and copy the following code into it:
