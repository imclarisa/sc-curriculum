...
5. Navigo provides us with the functionality to handle `<a>` tag links/hrefs much more simply than handling them with `afterRender`.
   ...
   > Note: Now that we have `data-navigo` which handles the click events for the nav links, we should remove any 'click event handlers' we might have added, so they don't conflict.
