...
5. Navigo provides us with the functionality to handle `<a>` tag links/hrefs much more simply than handling them with `afterRender`.
   ...
   2. In order for the function above to take effect, we must alter our nav bar HTML a bit.
      ...
      ```
   3. Now that we have `data-navigo` which handles the click event handler for the nav link, we should remove the event handlers we added early, so they don't conflict.
   
      In the `index.js` file within the route hooks before callback, remove the following:
