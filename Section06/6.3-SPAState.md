...
10. Our render function is accepting a piece of state and calling our functional components with pieces of state, however, we need to change our functional components to know what to do with that information.
    ...
    - Let's transform the way our NAV works. We'll "refactor our code" (rewrite it in a better format), make it easier to maintain, and make if more 'functional'.
      ...
      export default [
        ...
        { title: "Form", text: "Submit the Form" },
      ];
      ```
...
2. In our root `index.js`, we can now remove the functionality that populated our gallery with pictures

3. Now we just need to change our form handler to listen for submissions, then add the submission to `state.Gallery.pictures`.
