...
If you need help running Jasmine commands in the terminal, enter the command "`jasmine --h`" at the command prompt to see some help and instructions on using Jasmine CLI options.

The "`jasmine --h`" command will output something like this...
...
Go to the spec file and tell Jasmine where to look for the code: In `helloWorld.spec.js` require helloWorld.js.
Build a test Spec using the "AAA pattern" -- Arrange-Act-Assert.
...
In the root of your project create a new file called `helloWorld.js`
...
5.  Next, in the same Bsh Console, run our custom command script `npm test`, and compare the output.
The test should pass and you should see green dots and output in the console that look like this...
    ...
    7 tests, 4 assertions, 0 failures, 0 skipped
...
Copy the code for "`MathUtils.js`" into our project, creating the new file called `MathUtils.js`.
...
2. Also create a testing spec "`mathUtils.spec.js`" in our `/spec` folder, and copy in that code.
...
3. While the structure for this test is setup differently from the previous 2 examples, it uses the same methods to test the functionality of the MathUtils.

4. Run `jasmine` in teh terminal again and see the results
